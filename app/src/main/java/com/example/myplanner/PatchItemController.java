...
public class PatchItemController implements Callback<ItemData> {
    ...
    public PatchItemController(ItemActivity activity, int week, int day, int slot, String title, String text, String color){
    ...
    public void patch(){
        if(AppState.getInstance().getAuthController().isConnected()){
            ...
            Call<ItemData> call = plannerAPI.patchItem(week, day, slot, title, text, color,
    ...
    public void onResponse(Call<ItemData> call, Response<ItemData> response){
        if(response.isSuccessful()) {
            ...
            if(response.code() == 401){
                AppState.getInstance().getAuthController().refresh(()->{patch();});
            }else {
